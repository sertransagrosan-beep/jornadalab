...
if files:
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        for conductor, df_conductor in kpis.groupby("conductor"):
            ...
            nombre_hoja2 = f"{vehiculos[0]}_{nombre_conductor}"[:31]

            bloques_cond.to_excel(writer, sheet_name=nombre_hoja2, index=False)
            auto_ajustar_excel(writer.sheets[nombre_hoja2], bloques_cond)

    # NOMBRE ARCHIVO
    fecha_max = pd.to_datetime(kpis["fecha"]).max()
    mes = fecha_max.strftime("%Y-%m")
